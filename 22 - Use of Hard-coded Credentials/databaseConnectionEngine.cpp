...
int processDatabaseConnection(const char* db_host, const char* db_user, const char* db_pass, const char* db_name) {
    ...
    int auth_result = executeDatabaseAuthentication(db_host, db_user, db_pass, db_name);
    int conn_result = executeDatabaseConnection();
...
int executeDatabaseAuthentication(const char* db_host, const char* db_user, const char* db_pass, const char* db_name) {
    ...
    if (!mysql_conn) {
        ...
        result << "MySQL initialization failed";
    ...
    std::stringstream result;
    result << "MySQL authentication completed successfully";
    std::cout << result.str() << std::endl;
...
int executeDatabaseConnection(void) {
    ...
    if (rc != LDAP_SUCCESS) {
        std::stringstream result;
        result << "LDAP initialization failed";
        std::cout << result.str() << std::endl;
        return 1;
    }
    ...
    std::stringstream result;
    result << "LDAP connection established successfully";
    std::cout << result.str() << std::endl;
